...
class SillyTavernAPI {
    ...
    async toggleCharacterFavorite(charAvatar, newFavState) {
        try {
            // 먼저 SillyTavern의 기존 즐겨찾기 버튼을 클릭 시도
            const favBtn = document.getElementById('favorite_button');
            
            // 현재 선택된 캐릭터와 일치하는지 확인
            const context = this.getContext();
            const currentChar = context?.characters?.[context?.characterId];
            
            if (currentChar && currentChar.avatar === charAvatar && favBtn) {
                // 현재 선택된 캐릭터면 기존 버튼 클릭
                favBtn.click();
                console.log('[API] Toggled favorite via button click');
                return true;
            }
            ...
            if (response.ok) {
                ...
                console.log('[API] Character favorite toggled via API:', charAvatar, newFavState);
                return true;
            }
            
            console.warn('[API] edit-attribute API failed, trying fallback');
            
            // 폴백: 전체 캐릭터 데이터 업데이트
            const characters = this.getCharacters();
            const char = characters.find(c => c.avatar === charAvatar);
            if (char) {
                const editResponse = await this.fetchWithRetry('/api/characters/edit', {
                    method: 'POST',
                    headers: this.getRequestHeaders(),
                    body: JSON.stringify({
                        avatar_url: charAvatar,
                        ch_name: char.name,
                        fav: newFavState.toString()
                    })
                });
            ...
            console.error('[API] Failed to toggle character favorite:', error);
            return false;
        }
    }
